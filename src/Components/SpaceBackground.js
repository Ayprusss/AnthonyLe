...
import { rand, lerp, clamp, buildStars, CRAFT, drawCraft } from './spacecraft';
...
const SpaceBackground = () => {
  ...
  useEffect(() => {
    ...
    // Spacecraft models (Path2D art + metadata) live in ./spacecraft.js so the
    // RocketHunt mini-game can reuse them. spawnRocket() picks one at random.
    ...
    const drawRocket = (r, now) => {
      ...
      drawCraft(ctx, r.modelIdx, now, starRgb);
      ctx.restore();
    };
